...
export default function AppDashboard() {
  ...
  // Live DreamDEX orderbook depth -- real bid/ask levels from the staging CLOB API,
  // never fabricated: if the API is unreachable the panel says so instead of showing numbers.
  type OrderLevel = { price: number; size: number };
  const [orderbook, setOrderbook] = useState<{ source: string; bids: OrderLevel[]; asks: OrderLevel[] }>({
    source: "loading",
    bids: [],
    asks: [],
  });
  useEffect(() => {
    ...
    const symbol = symbolByAsset[activeAsset];
    let cancelled = false;
    if (!symbol) {
      setOrderbook({ source: "no_market", bids: [], asks: [] });
      return;
    }
    async function syncOrderbook() {
      try {
        ...
        if (!cancelled) setOrderbook({ source: "unavailable", bids: [], asks: [] });
      }
    }
    syncOrderbook();
    const obTimer = setInterval(syncOrderbook, 8000);
    return () => {
      cancelled = true;
      ...
      <div ref={dashboardRef} className="space-y-4 sm:space-y-5 font-mono px-1 pt-4 sm:pt-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-5">
          ...
          <div className="lg:col-span-6 bg-[#0b101d] border border-slate-800 rounded-xl p-4 sm:p-5 space-y-3">
            <div className="flex items-center justify-between border-b border-slate-800/80 pb-2">
              <div>
                <h3 className="text-xs font-bold text-white uppercase tracking-wider">DreamDEX Live Order Book</h3>
                <p className="text-[9px] sm:text-[10px] text-slate-400">
                  Real spot-market depth ({activeAsset === "BTC" ? "WBTC" : activeAsset === "ETH" ? "WETH" : activeAsset}:USDso) -- not the binary event-contract book itself
                </p>
              </div>
              <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${
                orderbook.source === "live_dreamdex_staging"
                  ? "bg-emerald-500/10 text-emerald-300 border-emerald-500/30"
                  : orderbook.source === "loading"
            ...
            {orderbook.source === "live_dreamdex_staging" && orderbook.bids.length > 0 ? (
              <>
                  ...
                  <div className="bg-[#060911] p-2 sm:p-2.5 rounded border border-slate-800 space-y-1.5">
                    ...
                    <div className="space-y-1 text-[11px]">
                      {orderbook.bids.map((lvl, i) => (
                        <div key={i} className={`flex justify-between px-1.5 py-0.5 rounded ${i === 0 ? "bg-emerald-500/10" : "bg-emerald-500/5"}`}>
                          <span className={i === 0 ? "text-emerald-400 font-bold" : "text-emerald-400"}>${lvl.price.toFixed(2)}</span>
                          <span className="text-slate-400">{lvl.size.toLocaleString("en-US")} size</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="bg-[#060911] p-2 sm:p-2.5 rounded border border-slate-800 space-y-1.5">
                    ...
                    <div className="space-y-1 text-[11px]">
                      {orderbook.asks.map((lvl, i) => (
                        <div key={i} className={`flex justify-between px-1.5 py-0.5 rounded ${i === 0 ? "bg-rose-500/15 border-l-2 border-rose-400" : "bg-rose-500/5"}`}>
                          <span className={i === 0 ? "text-rose-400 font-bold" : "text-rose-400"}>${lvl.price.toFixed(2)}</span>
                          <span className={i === 0 ? "text-white font-bold" : "text-slate-400"}>{lvl.size.toLocaleString("en-US")}{i === 0 ? " (BEST)" : " size"}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex justify-between text-[10px] sm:text-[11px] text-slate-400 pt-1">
                  ...
                  <span>Venue: <strong className="text-white">DreamDEX Somnia L1 (live staging API)</strong></span>
                </div>
              </>
            ) : (
              <div className="bg-[#060911] border border-slate-800 rounded p-4 text-center text-[11px] text-slate-500">
                {orderbook.source === "loading"
                  ? "Fetching live order book from DreamDEX..."
                  : orderbook.source === "no_market"
                  ? `No live DreamDEX market for ${activeAsset} yet -- switch to BTC, ETH, or SOMI to see real depth.`
                  : "DreamDEX's staging orderbook API is unreachable right now -- showing no data rather than a fake book."}
              </div>
            )}
          </div>
        </div>
